...
export default async function GuestOpportunityPage({ params }: { params: { id: string } }) {
  ...
  const tempMap: Record<string, any> = {
    'temp-general-ict': {
      ...
      reviews: [{ name: 'Amaya K', rating: 5, comment: 'Excellent practical course — learned a lot.' }],
    ...
    'temp-web-development': {
      ...
      reviews: [{ name: 'Liam R', rating: 5, comment: 'Great instructors and hands-on projects.' }],
      created_at: new Date().toISOString(),
    },
    'temp-english': {
      id: 'temp-english',
      ...
      reviews: [{ name: 'Sara P', rating: 5, comment: 'Helped my speaking and confidence a lot.' }],
      created_at: new Date().toISOString(),
    },
    'temp-photoshop': {
      id: 'temp-photoshop',
      ...
      reviews: [{ name: 'Noah M', rating: 5, comment: 'Practical and clear — great for beginners.' }],
      created_at: new Date().toISOString(),
    },
    'temp-skill-development': {
      ...
      reviews: [{ name: 'Olivia T', rating: 5, comment: 'Useful tips and excellent coaching.' }],
      created_at: new Date().toISOString(),
    },
    'temp-video-editing': {
      ...
      reviews: [{ name: 'Ethan V', rating: 5, comment: 'Great practical lessons and tips for quick editing.' }],
      created_at: new Date().toISOString(),
    },
  }
  ...
  function getImageFor(oppId: string) {
    if (oppId.startsWith('temp-general')) return 'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1200&q=60'
    if (oppId.startsWith('temp-web')) return 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=1200&q=60'
    if (oppId.startsWith('temp-english')) return 'https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=1200&q=60'
    if (oppId.startsWith('temp-photoshop')) return 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&w=1200&q=60'
    if (oppId.startsWith('temp-skill-development')) return 'https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=1200&q=60'
  ...
  if (!data) {
    return (
      <div className="flex flex-col flex-1 max-w-3xl mx-auto p-4 sm:p-6">
        ...
        <Link href="/cob/guest/opportunities" className="mt-4 text-purple-500 hover:underline">← Back to opportunities</Link>
      </div>
    )
  }
    ...
    <div className="flex flex-col flex-1 max-w-3xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-gray-200 dark:border-zinc-800">
        ...
        <div className="mt-6">
          <Link
            href="/cob/auth/login"
            className="w-full inline-flex justify-center items-center gap-2 py-3 px-6 rounded-xl text-sm font-bold text-white bg-purple-600 hover:bg-purple-500 transition-all shadow-lg shadow-purple-600/20"
          >
            Sign In to Apply
          </Link>
        </div>
